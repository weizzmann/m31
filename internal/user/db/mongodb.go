...
func (d *db) DeleteFriend(ctx context.Context, id string) error {
	filter := bson.M{}
	statement := bson.M{"$pull": bson.M{"friends": bson.M{"$in": bson.A{id}}}}
	result, err := d.collection.UpdateMany(ctx, filter, statement)
	if err != nil {
		return fmt.Errorf("failed to execute update user friends query. error: %v", err)
	}

	d.logger.Tracef("Updeted %d documents", result.ModifiedCount)

	return nil
}

func NewStorage(logger *logging.Logger, database *mongo.Database, collection string) user.Storage {
	return &db{
